...
export default function DesempenhoPage() {
  ...
  const maxUsuariosAtivos = useMemo(() => {
    if (!desempenhoData.porTurno || desempenhoData.porTurno.length === 0) {
      return 1; // Avoid division by zero
    }
    return Math.max(...desempenhoData.porTurno.map(t => t['Usuários Ativos']), 1);
